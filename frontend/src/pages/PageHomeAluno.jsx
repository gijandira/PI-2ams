...
export default function PageHomeAluno({ navigate }) {
  const modules = [
    { page:'comunicacao', icon:'💬', label:'Comunicação', color:'card-blue'   },
    { page:'licoes',      icon:'🎓', label:'Lições',      color:'card-green'  },
    { page:'agenda',      icon:'📅', label:'Agenda',      color:'card-pink'   },
    { page:'perfil',      icon:'👤', label:'Perfil',      color:'card-red'    },
    { page:'config',      icon:'⚙️', label:'Configurações',color:'card-yellow'},
    { page:'ia',          icon:'🤖', label:'Assistente IA',color:'card-dark'  },
  ];
    ...
    <>
      ...
      <div className="ha-mobile" style={{ minHeight:'100vh', display:'flex', flexDirection:'column', paddingTop:60 }}>
        ...
        <div style={{ flex:1, padding:'0 16px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, alignContent:'center' }}>
          {modules.map((m, i) => (
            <div key={i} className={`module-card ${m.color}`} onClick={() => navigate(m.page)}>
      ...
      <div className="ha-desktop" style={{ display:'none' }}>
        <nav className="desktop-sidebar">
          ...
          {[
            ...
            { icon:<IconRobot/>,    label:'Assistente IA', active:false, page:null           },
          ].map((item, i) => (
        ...
        <div className="main-content">
          <div className="page-wrapper">
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:20, marginBottom:32 }}>
              {modules.map((m, i) => (
                <div key={i} className={`module-card module-card-desktop ${m.color}`} style={{ padding:'28px 16px 22px', cursor:'pointer' }} onClick={() => navigate(m.page)}>
