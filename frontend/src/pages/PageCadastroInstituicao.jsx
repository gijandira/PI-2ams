...
const handleCadastroInstituicao = async () => {
  ...
  try {
    ...
    // Validar espaços em email e senha
    ...
    const response = await fetch(
      'http://localhost:3001/auth/cadastro-instituicao',
      {
        ...
        body: JSON.stringify({
          ...
          email: rawEmail,
          senha: rawSenha,
          confirmarsenha: rawConfirmarSenha,
          codigo: form.codigo
        })
      }
    );
    ...
    <>
      ...
      <div className="cadi-mobile" style={{ minHeight:'100vh', background:'var(--white)', display:'flex', flexDirection:'column', paddingTop:60 }}>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'18px 24px 10px', display:'flex', flexDirection:'column', gap:13 }}>
          ...
          <div className="field input-icon-wrap"><label>Crie uma senha:</label><div className="input-group"><input type={mostrarSenha ? 'text' : 'password'} placeholder="Crie uma senha..." value={form.senha}onChange={(e) =>setForm({...form,senha: e.target.value})}/><span className="field-icon" onClick={() => setMostrarSenha(!mostrarSenha)}>{mostrarSenha ? '🙈' : '👁️'}</span></div></div>
          <div className="field input-icon-wrap"><label>Confirme a senha:</label><div className="input-group"><input type={mostrarConfirmarSenha ? 'text' : 'password'} placeholder="Confirme a senha..." value={form.confirmarsenha}onChange={(e) =>setForm({...form,confirmarsenha: e.target.value})}/><span className="field-icon" onClick={() => setMostrarConfirmarSenha(!mostrarConfirmarSenha)}>{mostrarConfirmarSenha ? '🙈' : '👁️'}</span></div></div>
          ...
          <div style={{ width:'100%', maxWidth:500 }}>
            ...
            <div style={{ background:'var(--white)', borderRadius:28, padding:36, boxShadow:'0 8px 40px rgba(0,0,0,.09)' }}>
              ...
              <div className="grid-2">
                ...
                <div className="field"><label>E-mail institucional:</label><input type="email" placeholder="email@instituicao.com"value={form.email}onChange={(e) =>setForm({...form,email: e.target.value})} /></div>
                <div className="field input-icon-wrap"><label>Crie uma senha:</label><div className="input-group"><input type={mostrarSenha ? 'text' : 'password'} placeholder="••••••••" value={form.senha}onChange={(e) =>setForm({...form,senha: e.target.value})} /><span className="field-icon" onClick={() => setMostrarSenha(!mostrarSenha)}>{mostrarSenha ? '🙈' : '👁️'}</span></div></div>
                <div className="field input-icon-wrap"><label>Confirme a senha:</label><div className="input-group"><input type={mostrarConfirmarSenha ? 'text' : 'password'} placeholder="••••••••"value={form.confirmarsenha}onChange={(e) =>setForm({...form,confirmarsenha: e.target.value})} /><span className="field-icon" onClick={() => setMostrarConfirmarSenha(!mostrarConfirmarSenha)}>{mostrarConfirmarSenha ? '🙈' : '👁️'}</span></div></div>
