...
export default function PageLogin({ navigate, initialTab = 'responsavel' }) {
  ...
  // Limpa os campos ao mudar de aba para evitar misturar credenciais
      ...
      <div className="login-mobile-wrap" style={{ minHeight:'100vh', background:'var(--bg)', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ margin:'14px 24px 0' }}>
          <div className="profile-tabs">
            <button className={`tab-btn ${tab==='responsavel'?'active':''}`} onClick={() => alternarAba('responsavel')}>👤 Responsável</button>
            <button className={`tab-btn ${tab==='instituicao'?'active':''}`} onClick={() => alternarAba('instituicao')}>🏫 Instituição</button>
          </div>
        </div>
              ...
              {tab === 'responsavel' && (
                <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
                  <div style={{ display:'flex', flexDirection:'column', gap:16, marginBottom:8 }}>
                    <div className="field"><label>E-mail:</label><input className="login-input-focus" type="email" placeholder="seu@email.com" value={email} onChange={(e) => setEmail(e.target.value)}/></div>
                    <div>
                  ...
                  <div className="social-row" style={{ marginBottom:22 }}>
                    <button className="btn-social btn-google" style={{ fontSize:13 }} onClick={() => exibirMensagem('erro', 'Em breve', 'Login com Google ainda não disponível.')}>
                    ...
                    <button className="btn-social btn-facebook" style={{ fontSize:13 }} onClick={() => exibirMensagem('erro', 'Em breve', 'Login com Facebook ainda não disponível.')}>
              ...
              {tab === 'instituicao' && (
                <div>
                  <div style={{ display:'flex', flexDirection:'column', gap:16, marginBottom:16 }}>
                    <div className="field"><label>E-mail institucional:</label><input className="login-input-focus" type="email" placeholder="escola@instituicao.com" value={email}  onChange={(e) => setEmail(e.target.value)}/></div>
                    <div>
                      <div className="field input-icon-wrap" style={{ marginBottom:6 }}><label>Senha:</label><input className="login-input-focus" type={mostrarSenha ? 'text' : 'password'} placeholder="••••••••" value={senha} onChange={(e) => setSenha(e.target.value)} /><span className="field-icon" onClick={() => setMostrarSenha(!mostrarSenha)}>{mostrarSenha ? '🙈' : '👁️'}</span></div>
                      <div style={{ textAlign:'right' }}><span className="forgot-link" onClick={() => navigate('recuperar-senha')}>Esqueci minha senha</span></div>
                    </div>
