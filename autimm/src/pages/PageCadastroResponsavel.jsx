...
export default function PageCadastroResponsavel({ navigate }) {
  ...
  const [tipoCadastro, setTipoCadastro] = useState('responsavel');
  const [notificacao, setNotificacao] = useState(null);
  const [mostrarSenha, setMostrarSenha] = useState(false);
  const [mostrarConfirmarSenha, setMostrarConfirmarSenha] = useState(false);
  const [fotoAluno, setFotoAluno] = useState(null);
  const [fotoAlunoPreview, setFotoAlunoPreview] = useState(null);
  ...
  const handleFotoAlunoChange = (event) => {
    const file = event.target.files?.[0];
    if (!file) return;
    setFotoAluno(file);
    const preview = URL.createObjectURL(file);
    setFotoAlunoPreview(preview);
  };
...
const handleCadastro = async () => {
  ...
  try {
    ...
    const rawSenha = form.senha.trim();
    const rawConfirmarSenha = form.confirmarSenha.trim();
    ...
    if (!rawEmail) camposVazios.push('E-mail');
    if (!rawSenha) camposVazios.push('Senha');
    if (!rawConfirmarSenha) camposVazios.push('Confirmação de senha');
    ...
    if (/[\s]/.test(rawEmail) || /[\s]/.test(rawSenha) || /[\s]/.test(rawConfirmarSenha)) {
      setNotificacao({
        tipo: 'erro',
        titulo: 'E-mail e senha não podem conter espaços',
        mensagens: ['Remova espaços do e-mail e da senha antes de continuar']
      });
      return;
    }

    if (rawSenha !== rawConfirmarSenha) {
      setNotificacao({
        tipo: 'erro',
        titulo: 'As senhas não coincidem',
        mensagens: ['Verifique se os campos de senha estão idênticos']
      });
      return;
    }
    ...
    formData.append('senha', rawSenha);
    formData.append('tipo', tipoCadastro);
    
    if (fotoAluno) {
      formData.append('fotoAluno', fotoAluno);
    }
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'18px 24px 10px', display:'flex', flexDirection:'column', gap:13 }}>
          <label className="avatar-upload" style={{ cursor:'pointer' }}>
            <div className="avatar-circle" style={{ overflow:'hidden' }}>
              {fotoAlunoPreview ? (
                <img src={fotoAlunoPreview} alt="Prévia do aluno" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
              ) : '📷'}
            </div>
            <div className="avatar-label">Foto de perfil do aluno</div>
            <input type="file" accept="image/*" style={{ display: 'none' }} onChange={handleFotoAlunoChange} />
          </label>
          ...
          <div className="field"><label>Telefone:</label><input type="tel" placeholder="(00) 00000-0000" value={form.telefone} onChange={(e) => {
            const digits = e.target.value.replace(/\D/g, '').slice(0, 11);
            setForm({ ...form, telefone: digits });
          }} /></div>
          ...
          <div className="field input-icon-wrap">
            ...
            <div className="input-group">
              <input type={mostrarSenha ? 'text' : 'password'} placeholder="Crie uma senha..." value={form.senha} onChange={(e) => setForm({ ...form, senha: e.target.value })} />
              <span className="field-icon" onClick={() => setMostrarSenha(!mostrarSenha)}>{mostrarSenha ? '🙈' : '👁️'}</span>
            </div>
          </div>
          <div className="field input-icon-wrap">
            <label>Confirme a senha:</label>
            <div className="input-group">
              <input type={mostrarConfirmarSenha ? 'text' : 'password'} placeholder="Confirme a senha..." value={form.confirmarSenha} onChange={(e) => setForm({ ...form, confirmarSenha: e.target.value })} />
              <span className="field-icon" onClick={() => setMostrarConfirmarSenha(!mostrarConfirmarSenha)}>{mostrarConfirmarSenha ? '🙈' : '👁️'}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex:'0 0 52%', minHeight:'100vh', background:'#f6faff', display:'flex', alignItems:'center', justifyContent:'center', padding:'48px 64px', overflowY:'auto' }}>
          <div style={{ width:'100%', maxWidth:500 }}>
            ...
            <div style={{ background:'var(--white)', borderRadius:28, padding:36, boxShadow:'0 8px 40px rgba(0,0,0,.09)' }}>
              <label className="avatar-upload" style={{ cursor:'pointer', display:'flex', alignItems:'center', gap:18, padding:'16px 20px', background:'#f0f7ff', borderRadius:18, border:'1.5px dashed var(--blue)', marginBottom:24 }}>
                <div className="avatar-circle" style={{ width:68, height:68, fontSize:28, flexShrink:0, margin:0, overflow:'hidden' }}>
                  {fotoAlunoPreview ? (
                    <img src={fotoAlunoPreview} alt="Prévia do aluno" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                  ) : '📷'}
                </div>
